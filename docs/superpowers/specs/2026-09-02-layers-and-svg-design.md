...
- Framing: auto-sized pages are framed with `EntityBounds`, which applies the wipeout pixel mapping and solid OCS normals the renderer uses and skips entities whose bounds ACadSharp cannot compute; an `Insert` without a block is skipped with a Warning everywhere.
